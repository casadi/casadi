...
namespace casadi {
  extern "C" int CASADI_ROOTFINDER_BISECTION_EXPORT
  casadi_register_rootfinder_bisection(Rootfinder::Plugin *plugin) {
  ...
  const Options Bisection::options_ = {{&Rootfinder::options_},
    {
      ...
      {"max_iter",
       {OT_INT, "Maximum number of Newton iterations to perform before returning."}},
  ...
  void Bisection::init(const Dict &opts) {
    ...
    casadi_assert(oracle_.n_in() > 0,
                  "Bisection: the supplied f must have at least one input.");
  ...
  void Bisection::set_work(void *mem, const double **&arg, double **&res,
  ...
  int Bisection::solve(void *mem) const {
    ...
    for (m->search_iter = 1; m->search_iter <= max_search_; ++m->search_iter) {
      ...
      if (std::isnan(fa) || std::isnan(fb))
        return finish(m, a, fa, b - a, -1, false, SOLVER_RET_UNKNOWN);
  ...
  int Bisection::finish(BisectionMemory *m, double x_sol, double f_sol, double width,
  ...
  Dict Bisection::get_stats(void *mem) const {
    ...
    stats["search_iter"] = m->search_iter; // number of bracket-search steps taken
