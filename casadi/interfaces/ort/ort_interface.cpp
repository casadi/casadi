...
namespace casadi {
  ...
  OnnxRuntimeInterface::OnnxRuntimeInterface(const std::string& name,
    ...
    data_.session = nullptr; data_.env = nullptr; data_.mem = nullptr;
  }

  OnnxRuntimeInterface::~OnnxRuntimeInterface() {
    if (data_.mem) ort_api_->ReleaseMemoryInfo(data_.mem);
    if (data_.session) ort_api_->ReleaseSession(data_.session);
    if (data_.env) ort_api_->ReleaseEnv(data_.env);
  }
  ...
  void OnnxRuntimeInterface::init(const Dict& opts) {
    ...
    build_prob();
    casadi_assert(casadi_onnxruntime_init(&data_, &prob_) == 0,
  ...
  int OnnxRuntimeInterface::eval(const double** arg, double** res,
    ...
    return casadi_onnxruntime_solve(&data_, &prob_, arg, res);
  }
  ...
  void OnnxRuntimeInterface::codegen_body(CodeGenerator& g) const {
    ...
    g << "static struct casadi_onnxruntime_data data = {0, 0, 0};\n";
