...
namespace casadi {
  ...
  SundialsSimulator::SundialsSimulator(DeserializingStream& s) : Simulator(s) {
    int version = s.version("SundialsSimulator", 1, 2);
    s.unpack("SundialsSimulator::abstol", abstol_);
    s.unpack("SundialsSimulator::reltol", reltol_);
    s.unpack("SundialsSimulator::max_num_steps", max_num_steps_);
    s.unpack("SundialsSimulator::stop_at_end", stop_at_end_);
    s.unpack("SundialsSimulator::quad_err_con", quad_err_con_);
    s.unpack("SundialsSimulator::steps_per_checkpoint", steps_per_checkpoint_);
    s.unpack("SundialsSimulator::disable_internal_warnings", disable_internal_warnings_);
    s.unpack("SundialsSimulator::max_multistep_order", max_multistep_order_);
    s.unpack("SundialsSimulator::linear_solver", linear_solver_);
    s.unpack("SundialsSimulator::linear_solver_options", linear_solver_options_);

    s.unpack("SundialsSimulator::max_krylov", max_krylov_);
    s.unpack("SundialsSimulator::use_precon", use_precon_);
    s.unpack("SundialsSimulator::second_order_correction", second_order_correction_);

    s.unpack("SundialsSimulator::step0", step0_);
    if (version>=2) {
      s.unpack("SundialsSimulator::max_step_size", max_step_size_);
    } else {
      max_step_size_ = 0;
    }

    s.unpack("SundialsSimulator::nonlin_conv_coeff", nonlin_conv_coeff_);
    s.unpack("SundialsSimulator::max_order", max_order_);

    s.unpack("SundialsSimulator::linsolF", linsolF_);
    s.unpack("SundialsSimulator::linsolB", linsolB_);

    int newton_scheme;
    s.unpack("SundialsSimulator::newton_scheme", newton_scheme);
    newton_scheme_ = static_cast<NewtonScheme>(newton_scheme);

    int interp;
    s.unpack("SundialsSimulator::interp", interp);
    interp_ = static_cast<InterpType>(interp);

  }

  void SundialsSimulator::serialize_body(SerializingStream &s) const {
    Simulator::serialize_body(s);
    s.version("SundialsSimulator", 2);
    s.pack("SundialsSimulator::abstol", abstol_);
    s.pack("SundialsSimulator::reltol", reltol_);
    s.pack("SundialsSimulator::max_num_steps", max_num_steps_);
    s.pack("SundialsSimulator::stop_at_end", stop_at_end_);
    s.pack("SundialsSimulator::quad_err_con", quad_err_con_);
    s.pack("SundialsSimulator::steps_per_checkpoint", steps_per_checkpoint_);
    s.pack("SundialsSimulator::disable_internal_warnings", disable_internal_warnings_);
    s.pack("SundialsSimulator::max_multistep_order", max_multistep_order_);

    s.pack("SundialsSimulator::linear_solver", linear_solver_);
    s.pack("SundialsSimulator::linear_solver_options", linear_solver_options_);
    s.pack("SundialsSimulator::max_krylov", max_krylov_);
    s.pack("SundialsSimulator::use_precon", use_precon_);
    s.pack("SundialsSimulator::second_order_correction", second_order_correction_);

    s.pack("SundialsSimulator::step0", step0_);
    s.pack("SundialsSimulator::max_step_size", max_step_size_);

    s.pack("SundialsSimulator::nonlin_conv_coeff", nonlin_conv_coeff_);
    s.pack("SundialsSimulator::max_order", max_order_);

    s.pack("SundialsSimulator::linsolF", linsolF_);
    s.pack("SundialsSimulator::linsolB", linsolB_);

    s.pack("SundialsSimulator::newton_scheme", static_cast<int>(newton_scheme_));
    s.pack("SundialsSimulator::interp", static_cast<int>(interp_));
  }
