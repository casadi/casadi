...
extern "C"
{
  int mb03vd_(f_int* n, f_int* p, f_int* ilo, f_int* ihi, double *a, f_int* lda1, f_int* lda2, double* tau, f_int* ldtau, double* dwork, f_int *info);
  int mb03vy_(f_int* n, f_int* p, f_int* ilo, f_int* ihi, double *a, f_int* lda1, f_int* lda2, const double* tau, f_int* ldtau, double* dwork, f_int *ld_work, f_int *info );
...
if (dwork==0) {
       ...
       slicot_mb03wd(job,compz,n,p,ilo,ihi,iloz,ihiz,h,ldh1,ldh2,z,ldz1,ldz2,wr,wi, &work[0],ihi-ilo+p-1);
       return;
     }
     f_int n_ = n;
     f_int p_ = p;
   ...
   void slicot_periodic_schur(int n, int K, const std::vector< double > & a, std::vector< double > & t,  std::vector< double > & z, std::vector<double> &dwork,std::vector<double> &eig_real, std::vector<double> &eig_imag) {
    ...
    if (dwork.size()==0) {
      ...
      casadi_warning("You provided working memory for slicot_periodic_schur, but it is insufficient. We need size " << mem_needed << " but supplied size is " << dwork.size() <<".");
      dwork.resize(mem_needed);
    } else {
