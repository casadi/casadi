...
SXFunctionNode::SXFunctionNode(const vector<SXMatrix>& inputv_, const vector<SXMatrix>& outputv_) : inputv(inputv_),  outputv(outputv_) {
...
SXFunctionNode::~SXFunctionNode(){
}


void SXFunctionNode::sort_depth_first(stack<SXNode*>& s, vector<BinarySXNode*>& algnodes){
...
void SXFunctionNode::resort_bredth_first(vector<BinarySXNode*>& algnodes){
...
void SXFunctionNode::resort_postpone(vector<BinarySXNode*>& algnodes, vector<int>& lind){
...
void SXFunctionNode::evaluate(int fsens_order, int asens_order){
...
SXMatrix SXFunctionNode::hess(int iind, int oind){
  if(output_.at(oind).numel() != 1)
    throw CasadiException("SXFunctionNode::hess: function must be scalar");
...
SXMatrix SXFunctionNode::grad(int iind, int oind){
  return trans(jac(iind,oind));
}

SXMatrix SXFunctionNode::jac(int iind, int oind){
...
return ret;
    ...
    throw CasadiException("SXFunctionNode::jac: Unknown ad_mode");
  }
}

bool SXFunctionNode::isSmooth() const{
...
void SXFunctionNode::printAlgorithm(ostream &stream) const{
 for(vector<AlgEl>::const_iterator it = algorithm.begin(); it!=algorithm.end(); ++it){
    int op = it->op;
    stringstream s,s0,s1;
    s << "i_" << it->ind;
...
void SXFunctionNode::generateCode(const string& src_name) const{
   // Output
...
SXFunction::SXFunction(const SXMatrix& arg, const SXMatrix& res){
  ...
  assignNode(new SXFunctionNode(argv,resv));
}
...
SXFunction::SXFunction(const vector< SXMatrix>& arg, const vector<SXMatrix>& res){
  assignNode(new SXFunctionNode(arg,res));
}
...
SXFunction::SXFunction(const vector< SXMatrix>& arg, const SXMatrix& res){
  vector<SXMatrix> resv(1,res);
  assignNode(new SXFunctionNode(arg,resv));
}
...
void SXFunction::createNode(const vector<SXMatrix>& ip, const vector<SXMatrix>& op){
  assignNode(new SXFunctionNode(ip,op));
}
#endif

const SXFunctionNode* SXFunction::operator->() const{
  return (const SXFunctionNode*)FX::operator->();
}

SXFunctionNode* SXFunction::operator->(){
  return (SXFunctionNode*)FX::operator->();
}

void SXFunctionNode::init(){
...
void SXFunctionNode::print(ostream &stream) const{
  stream << "sx function(\"" << getOption("name") << "\")";
}

FX SXFunctionNode::jacobian(int iind, int oind){
  if(getOption("symbolic_jacobian")==true){
    ...
    // numeric jacobian
    return FXNode::jacobian(iind,oind);
  }
}

FX SXFunctionNode::hessian(int iind, int oind){
...
void SXFunctionNode::eval(const vector<SXMatrix>& input_s, vector<SXMatrix>& output_s){
...
SXFunctionNode* SXFunctionNode::clone() const{
  return new SXFunctionNode(*this);
}
