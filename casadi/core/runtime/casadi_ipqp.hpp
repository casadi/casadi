...
// SYMBOL "ipqp_reset"
template<typename T1>
int casadi_ipqp_reset(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_pr"
template<typename T1>
void casadi_ipqp_pr(casadi_ipqp_data<T1>* d) {
...
// SYMBOL "ipqp_du"
template<typename T1>
void casadi_ipqp_du(casadi_ipqp_data<T1>* d) {
  // Calculate largest constraint violation
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
  d->du = 0;
...
// SYMBOL "ipqp_du_check"
template<typename T1>
int casadi_ipqp_du_check(casadi_ipqp_data<T1>* d, casadi_int i) {
  // Local variables
  casadi_int k;
  T1 new_du;
...
// SYMBOL "ipqp_du_index"
template<typename T1>
void casadi_ipqp_du_index(casadi_ipqp_data<T1>* d) {
  ...
  for (i = 0; i < p->nz; ++i) {
    ...
    if (d->lam[i] == 0) {
      ...
      if (!casadi_ipqp_du_check(d, i)) continue;
    }
...
// SYMBOL "ipqp_pr_index"
template<typename T1>
void casadi_ipqp_pr_index(casadi_ipqp_data<T1>* d) {
...
// SYMBOL "ipqp_kkt"
template<typename T1>
void casadi_ipqp_kkt(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i, k;
...
// SYMBOL "ipqp_kkt_vector"
template<typename T1>
void casadi_ipqp_kkt_vector(casadi_ipqp_data<T1>* d, T1* kkt_i, casadi_int i) {
  // Local variables
  casadi_int k;
  const casadi_int *h_colind, *h_row, *a_colind, *a_row, *at_colind, *at_row;
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_kkt_dot"
template<typename T1>
T1 casadi_ipqp_kkt_dot(casadi_ipqp_data<T1>* d, const T1* v, casadi_int i) {
  // Local variables
  casadi_int k;
  const casadi_int *h_colind, *h_row, *a_colind, *a_row, *at_colind, *at_row;
  T1 r;
...
// SYMBOL "ipqp_kkt_residual"
template<typename T1>
void casadi_ipqp_kkt_residual(casadi_ipqp_data<T1>* d, T1* r) {
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
  for (i=0; i<p->nz; ++i) {
    if (d->lam[i]>0.) {
...
// SYMBOL "ipqp_zero_blocking"
template<typename T1>
int casadi_ipqp_zero_blocking(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  T1 dz_max = 0;
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_primal_blocking"
template<typename T1>
void casadi_ipqp_primal_blocking(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  T1 trial_z;
  const casadi_ipqp_prob<T1>* p = d->prob;
  // Check if violation with tau=0 and not improving
  if (casadi_ipqp_zero_blocking(d)) {
    d->tau = 0.;
    return;
  }
...
// SYMBOL "ipqp_dual_breakpoints"
template<typename T1>
casadi_int casadi_ipqp_dual_breakpoints(casadi_ipqp_data<T1>* d, T1* tau_list,
...
// SYMBOL "ipqp_dual_blocking"
template<typename T1>
casadi_int casadi_ipqp_dual_blocking(casadi_ipqp_data<T1>* d) {
  // Local variables
  ...
  n_tau = casadi_ipqp_dual_breakpoints(d, d->w, d->iw, d->tau);
...
// SYMBOL "ipqp_take_step"
template<typename T1>
void casadi_ipqp_take_step(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
  // Get current sign
...
// SYMBOL "ipqp_flip_check"
template<typename T1>
int casadi_ipqp_flip_check(casadi_ipqp_data<T1>* d) {
  const casadi_ipqp_prob<T1>* p = d->prob;
  // Calculate the difference between unenforced and enforced column index
  casadi_ipqp_kkt_vector(d, d->dlam, d->index);
...
// SYMBOL "ipqp_factorize"
template<typename T1>
void casadi_ipqp_factorize(casadi_ipqp_data<T1>* d) {
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_expand_step"
template<typename T1>
void casadi_ipqp_expand_step(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "casadi_ipqp_pr_direction"
template<typename T1>
int casadi_ipqp_pr_direction(casadi_ipqp_data<T1>* d) {
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
  for (i=0; i<p->nz; ++i) {
...
// SYMBOL "casadi_ipqp_du_direction"
template<typename T1>
int casadi_ipqp_du_direction(casadi_ipqp_data<T1>* d) {
  casadi_int i;
  const casadi_ipqp_prob<T1>* p = d->prob;
  for (i=0; i<p->nx; ++i) {
...
// SYMBOL "ipqp_enforceable"
template<typename T1>
int casadi_ipqp_enforceable(casadi_ipqp_data<T1>* d, casadi_int i, casadi_int s) {
  // Local variables
  casadi_int k;
  const casadi_int *at_colind, *at_row;
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_singular_step"
// C-REPLACE "static_cast<T1*>(0)" "0"
template<typename T1>
int casadi_ipqp_singular_step(casadi_ipqp_data<T1>* d) {
  // Local variables
  T1 tau_test, tau;
  ...
  for (k=0; k<nk; ++k) {
    ...
    for (i=0; i<p->nz; ++i) {
      d->iw[i] = d->lincomb[i] && fabs(casadi_ipqp_kkt_dot(d, d->dz, i)) > 1e-12;
    }
    // Calculate step, dz and dlam
    casadi_ipqp_expand_step(d);
    ...
    for (neg = 0; neg < 2; ++neg) {
      ...
      if (casadi_ipqp_pr_direction(d)) continue;
      // Make sure dual infeasibility doesn't exceed limits
      if (casadi_ipqp_du_direction(d)) continue;
...
// SYMBOL "ipqp_calc_step"
template<typename T1>
int casadi_ipqp_calc_step(casadi_ipqp_data<T1>* d) {
  // Local variables
  const casadi_ipqp_prob<T1>* p = d->prob;
  // Reset returns
  d->r_index = -1;
  d->r_sign = 0;
  // Handle singularity
  if (d->sing) return casadi_ipqp_singular_step(d);
  // Negative KKT residual
  casadi_ipqp_kkt_residual(d, d->dz);
...
// SYMBOL "ipqp_calc_sens"
template<typename T1>
void casadi_ipqp_calc_sens(casadi_ipqp_data<T1>* d, casadi_int i) {
  // Local variables
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_calc_dependent"
template<typename T1>
void casadi_ipqp_calc_dependent(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int i;
  T1 r;
  const casadi_ipqp_prob<T1>* p = d->prob;
  ...
  casadi_ipqp_calc_sens(d, d->idu);
}

// SYMBOL "ipqp_linesearch"
template<typename T1>
void casadi_ipqp_linesearch(casadi_ipqp_data<T1>* d) {
  // Local variables
  casadi_int du_index;
...
void casadi_ipqp_flip(casadi_ipqp_data<T1>* d) {
  ...
  if (!d->sing && d->index == -1) {
    if (d->pr * p->dual_inf_tol >= p->constr_viol_tol * d->du) {
      ...
      if (d->pr >= p->constr_viol_tol) casadi_ipqp_pr_index(d);
    } else {
      // Improve dual feasibility if dominating
      if (d->du >= p->dual_inf_tol) casadi_ipqp_du_index(d);
    }
  }
  ...
  if (d->index >= 0) {
    ...
    if (!d->sing) d->has_search_dir = casadi_ipqp_flip_check(d);
    // Perform the active-set change
    d->lam[d->index] = d->sign==0 ? 0 : d->sign > 0 ? p->dmin : -p->dmin;
    // Recalculate primal and dual infeasibility
    casadi_ipqp_calc_dependent(d);
  }
}

// SYMBOL "ipqp_prepare"
template<typename T1>
int casadi_ipqp_prepare(casadi_ipqp_data<T1>* d) {
  // Local variables
  const casadi_ipqp_prob<T1>* p = d->prob;
...
// SYMBOL "ipqp_iterate"
template<typename T1>
int casadi_ipqp_iterate(casadi_ipqp_data<T1>* d) {
  // Reset message flag
  d->msg = 0;
  // Start a new iteration
  d->iter++;
  // Calculate search direction
  if (casadi_ipqp_calc_step(d)) {
    d->status = IPQP_NO_SEARCH_DIR;
    return 1;
  }
  // Line search in the calculated direction
  casadi_ipqp_linesearch(d);
  // Keep iterating
  return 0;
}
