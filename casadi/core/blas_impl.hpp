...
namespace casadi {
  ...
  /* Note: the strong <double> specialization for casadi_blas_mtimes
   * lives in blas.cpp; its declaration is in blas.hpp (which every
   * condensing-instantiating TU is required to include -- see
   * conic_impl.hpp).  No spec for the primary casadi_mtimes_dense<double>
   * -- that's the unspecialized triple loop, which Blas::mtimes(0, ...)
   * calls without recursion. */
