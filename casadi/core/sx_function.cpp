...
namespace casadi {
    ...
    for (std::vector<SXNode*>::iterator it=nodes.begin(); it!=nodes.end(); ++it) {
      ...
      if (output_indicator.find(n)!=output_indicator.end()) {
        for (int i : output_indicator[n]) {
          ...
          ae.i0 = out.first;
          ae.i1 = n->temp;
          ae.i2 = out.second*std::abs(stride_out_[ae.i0]);
  ...
  SX SXFunction::jac_ve(const Dict& opts) const {
    casadi_assert_dev(out_.size()==1);
    casadi_assert_dev(in_.size()==1 || in_.size()==2);
    // In case we have two inputs, assume the second want is is_diff_in false
    if (in_.size()==2) {
      casadi_assert_dev(!is_diff_in_[1]);
    }
    ...
    for (auto&& a : algorithm_) {
      switch (a.op) {
      case OP_INPUT:
        // Only involve the first input when doing the Jacobian
        if (a.i1 == 0) {
...
*/
  ...
  std::vector<SX> SXFunction::jac_alt(const Dict& opts) const {
    Dict ops;
    ops["live_variables"] = false;
    ops["allow_free"] = true;
    ops["cse"] = true;
    ops["is_diff_in"] = is_diff_in_;
    Function f("f", in_, out_, ops);
