...
namespace casadi {
  ...
  void HorzRepWeave::eval_mx(const std::vector<MX>& arg, std::vector<MX>& res,
      const std::vector<bool>& unique) const {
  ...
  void HorzRepWeave::generate(CodeGenerator& g,
    ...
    g << "casadi_weave(" << g.work(arg[0], nnz, arg_is_ref[0]) << ", "
      << nnz_ << ", " << m_ << ", " << n_ << ", " << g.work(res[0], nnz, res_is_ref[0]) << ");\n";
  }
