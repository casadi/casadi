...
namespace casadi {
  ...
  MX OutputNode::get_nzref(const Sparsity& sp, const std::vector<casadi_int>& nz, bool unique) const {
    if (dep().op()==OP_HORZSPLIT) {
      ...
      return dep().dep()->get_nzref(sp, nz_mod, false);
    } else {
      return MXNode::get_nzref(sp, nz, unique);
    }
  }
