...
namespace casadi {
  ...
  void ImplicitFunctionInternal::evalFwdNode(const MX& f,
    ...
    vector<MX> argv(f->ndep());
    for (int i=0; i<argv.size(); ++i) argv[i] = f->dep(i);
    MX z = f->getOutput(iout_);
  ...
  void ImplicitFunctionInternal::evalAdjNode(const MX& f, MXPtrVV& aseed, MXPtrVV& asens) {
    vector<MX> argv(f->ndep());
    for (int i=0; i<argv.size(); ++i) argv[i] = f->dep(i);
    MX z = f->getOutput(iout_);
    ...
    // Arguments when calling f/f_der
    vector<MX> v;
    v.reserve(getNumInputs() + getNumOutputs()*nadj);
    v.insert(v.end(), argv.begin(), argv.end());
    v[iin_] = z;
