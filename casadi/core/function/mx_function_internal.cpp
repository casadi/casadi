...
namespace casadi {
      ...
      if (it->op!=OP_OUTPUT) {
        for (int c=0; c<it->res.size(); ++c) {
          if (it->res[c]>=0) {
            ...
            if (work_[it->res[c]].isEmpty()) {
              work_[it->res[c]] = Matrix<double>::zeros(it->data->sparsity(c));
            }
          }
        }
      }
  ...
  void MXFunctionInternal::updatePointers(const AlgEl& el) {
    ...
    if (el.op!=OP_OUTPUT) {
      for (int i=0; i<mx_output_.size(); ++i) {
        if (el.res[i]>=0) {
          mx_output_[i] = work_[el.res[i]].ptr();
        } else {
          mx_output_[i] = 0;
        }
      }
  ...
  void MXFunctionInternal::evaluate() {
    ...
    for (vector<AlgEl>::iterator it=algorithm_.begin(); it!=algorithm_.end(); ++it, ++alg_counter) {
      ...
      if (it->op==OP_INPUT) {
        // Pass an input
        work_[it->res.front()].set(input(it->arg.front()));
      } else if (it->op==OP_OUTPUT) {
        // Get an output
        work_[it->arg.front()].get(output(it->res.front()));
      } else {
  ...
  void MXFunctionInternal::spInit(bool fwd) {
    ...
    for (vector<DMatrix>::iterator it=work_.begin(); it!=work_.end(); ++it) {
      //Get a pointer to the int array
      bvec_t *iwork = get_bvec_t(it->data());
      fill_n(iwork, it->nnz(), bvec_t(0));
    }
  }
  ...
  void MXFunctionInternal::spEvaluate(bool fwd) {
    if (fwd) { // Forward propagation
      ...
      for (vector<AlgEl>::iterator it=algorithm_.begin(); it!=algorithm_.end(); it++) {
        if (it->op==OP_INPUT) {
          // Pass input seeds
          vector<double> &w = work_[it->res.front()].data();
          ...
          vector<double> &w = work_[it->arg.front()].data();
          bvec_t* iwork = get_bvec_t(w);
  ...
  void MXFunctionInternal::printWork(ostream &stream) {
    for (int k=0; k<work_.size(); ++k) {
      stream << "work[" << k << "] = " << work_[k].data() << endl;
    }
  }
