...
namespace casadi {
  ...
  void Node::disp(std::ostream& stream, bool more) const {
    ...
    for (size_t k = 0; k < dimension.size(); ++k) {
      if (k) stream << "x";
      if (dimension[k] >= 0) stream << dimension[k];
      else
  ...
  std::vector<casadi_int> GraphBuilder::dimension(const std::string& name) const {
    return (*this)->node(name).dimension;
  }
  std::string GraphBuilder::dtype(const std::string& name) const {
    return (*this)->node(name).dtype;
  }
  std::vector<std::string> GraphBuilder::dimension_param(const std::string& name) const {
    return (*this)->node(name).dim_params;
  }
  ...
  void GraphBuilderInternal::populate_from_function() {
    ...
    for (casadi_int i = 0; i < fun_.n_in(); ++i) {
      ...
      n.io = "input";
      n.dimension = {fun_.size1_in(i), fun_.size2_in(i)};
      n.dim_params = {"", ""};
      nodes_.push_back(n);
    }
    for (casadi_int i = 0; i < fun_.n_out(); ++i) {
      ...
      n.io = "output";
      n.dimension = {fun_.size1_out(i), fun_.size2_out(i)};
      n.dim_params = {"", ""};
      nodes_.push_back(n);
    }
  }
  ...
  std::vector<casadi_int> GraphBuilderInternal::resolved_shape(const Node& n) const {
    ...
    for (size_t k = 0; k < n.dimension.size(); ++k) {
      casadi_int d = n.dimension[k];
  ...
  std::vector<std::string> GraphBuilderInternal::dynamic_params() const {
    ...
    for (const Node& n : nodes_) {
      for (size_t k = 0; k < n.dimension.size(); ++k) {
        if (n.dimension[k] < 0 && !n.dim_params[k].empty() &&
  ...
  void GraphBuilderInternal::bind_shape(const std::string& input_name,
    ...
    casadi_assert(shape.size() == t.dimension.size(),
    ...
    for (size_t k = 0; k < t.dimension.size(); ++k) {
      if (t.dimension[k] < 0 && !t.dim_params[k].empty()) dim_bindings_[t.dim_params[k]] = shape[k];
    }
  }
