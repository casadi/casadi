...
throw CasadiException("Error in PermuteLayout::" FNAME " "\
  ...
  MX PermuteLayout::get_nzref(const Sparsity& sp, const std::vector<casadi_int>& nz,
      bool unique) const {
  ...
  void PermuteLayout::eval_mx(const std::vector<MX>& arg, std::vector<MX>& res,
  ...
  casadi_int PermuteLayout::sz_self(casadi_int i) const {
    return relay_.target().size();
  }
  ...
  void PermuteLayout::generate(CodeGenerator& g,
    ...
    relay_.generate(g, g.work(arg[0], nnz(), arg_is_ref[0]), g.work(res[0], nnz(), res_is_ref[0]));
