...
namespace casadi {
  ...
  generate(CodeGenerator& g,
    ...
    if (n_dep()==3) {
      ...
      nrow_y = dep(2).size1();
      ncol_y = dep(2).size2();
      x = g.work(arg[1], dep(1).nnz(), arg_is_ref[1]);
      y = g.work(arg[2], dep(2).nnz(), arg_is_ref[2]);
    } else {
      nrow_x = dep(0).size1();
      ncol_x = dep(0).size2();
      nrow_y = dep(1).size1();
      ...
      x = g.work(arg[0], dep(0).nnz(),  arg_is_ref[0]);
      y = g.work(arg[1], dep(1).nnz(), arg_is_ref[1]);
    }
    ...
    if (g.blasfeo) {//flops>100) {
      ...
      if (nrow_x==1) {
        ...
        if (n_dep()==3) {
          g << "casadi_real sum = " << "("+g.work((arg[0]==res[0] ? res[0] : arg[0]), nnz(), (arg[0]==res[0] ? res_is_ref[0] : arg_is_ref[0]))+")[i];\n";
        } else {
          g << "casadi_real sum = 0.;\n";
        }
        ...
        g << "(" << g.work(res[0], nnz(), false) << ")[i] = sum;\n";
        g << "}\n";
        ...
        g  << " for (j=0; j<" << nrow_x << "; ++j) "
          << " (" << g.work(res[0], nnz(), false) << ")[j] += (" << x << ")[k*" << nrow_x << "+j]*tmp;\n";
        g << "}\n";
        /*
        ...
        g << "&blasfeo_beta,";
        g << g.work(res[0], dep(0).nnz(), false) << ",&blasfeo_ldc);\n";
      }
    } else {
      casadi_assert_dev(!trans_x_);
      casadi_assert_dev(!trans_y_);
