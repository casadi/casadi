...
namespace casadi {
  ...
  void MMin::generate(CodeGenerator& g,
    ...
    g << g.work(res[0], nnz()) << "[0] = " << g.mmin(g.work(arg[0], dep(0).nnz()), dep(0).nnz(), dep(0).is_dense()) << ";\n";
  }

  void MMax::generate(CodeGenerator& g,
                                const std::vector<casadi_int>& arg,
                                const std::vector<casadi_int>& res) const {
    g << g.work(res[0], nnz()) << "[0] = " << g.mmax(g.work(arg[0], dep(0).nnz()), dep(0).nnz(), dep(0).is_dense()) << ";\n";
  }
