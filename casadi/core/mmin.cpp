...
namespace casadi {
  ...
  void MMin::eval_mx(const std::vector<MX>& arg, std::vector<MX>& res, bool unique) const {
    res[0] = mmin(arg[0]);
  }

  void MMax::eval_mx(const std::vector<MX>& arg, std::vector<MX>& res, bool unique) const {
    res[0] = mmax(arg[0]);
  }
