...
namespace casadi {
  ...
  bool has_onnx(const std::string& solver) {
    return OnnxFunction::has_plugin(solver);
  }

  void load_onnx(const std::string& solver) {
    OnnxFunction::load_plugin(solver);
  }

  std::vector<std::string> onnx_solvers() {
    std::vector<std::string> ret;
    for (auto&& s : OnnxFunction::solvers_) ret.push_back(s.first);
    return ret;
  }

  std::string onnx_doc(const std::string& solver) {
