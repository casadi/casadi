...
namespace casadi {
  ...
  std::string CodeGenerator::generate(const std::string& prefix) {
    ...
    CodeGenerator::file_open(s, fullname, this->cpp);
    ...
    for (const auto& e : body_parts) {
      if (split) {
        ...
        CodeGenerator::file_open(ss, fullname, this->cpp);
    ...
    // Finalize file
    CodeGenerator::file_close(s, this->cpp);
    ...
    if (this->with_header) {
      ...
      CodeGenerator::file_open(s, prefix + this->name + ".h", this->cpp);
