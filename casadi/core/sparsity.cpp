...
namespace casadi {
  ...
  Sparsity Sparsity::ldl(std::vector<casadi_int>& parent) const {
  ...
  void Sparsity::symbfact(std::vector<casadi_int>& count, std::vector<casadi_int>& parent,
                          std::vector<casadi_int>& post, Sparsity& L, bool ata) const {
    // Dimensions
    casadi_int size1=this->size1(), size2=this->size2();
    // Allocate vectors
    ...
    if (ata) {
      ...
      const casadi_int* L_colind = L.colind();
      for (casadi_int i=0; i<size2; ++i) count[i] = L_colind[i+1] - L_colind[i];
    }
  }
