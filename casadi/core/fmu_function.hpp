...
template<typename T1>
struct casadi_finite_diff_mem_new {
  // Input precision
  T1 reltol;
  // Output precision
  T1 abstol;
...
template<typename T1>
T1 casadi_forward_diff_new(T1** yk, T1* y0, T1* J,
    T1 h, casadi_int n_y, const casadi_finite_diff_mem_new<T1>* m) {
  casadi_int i;
  for (i=0; i<n_y; ++i) {
    J[i] = (yk[0][i]-y0[i])/h;
  }
...
template<typename T1>
T1 casadi_central_diff_new(T1** yk, T1* y0, T1* J,
    T1 h, casadi_int n_y, const casadi_finite_diff_mem_new<T1>* m) {
  // Return value
  T1 u;
  // Stencil
...
template<typename T1>
T1 casadi_smoothing_diff_new(T1** yk, T1* y0, T1* J,
    T1 h, casadi_int n_y, const casadi_finite_diff_mem_new<T1>* m) {
  // Return value
  T1 u;
  // Stencil
