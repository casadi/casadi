...
namespace casadi {
  ...
  template<bool Tr>
  int Solve<Tr>::eval(const double** arg, double** res, int* iw, double* w, int mem) const {
  ...
  template<bool Tr>
  int Solve<Tr>::sp_forward(const bvec_t** arg, bvec_t** res, int* iw, bvec_t* w, int mem) const {
  ...
  template<bool Tr>
  int Solve<Tr>::sp_reverse(bvec_t** arg, bvec_t** res, int* iw, bvec_t* w, int mem) const {
    // Number of right-hand-sides
    int nrhs = dep(0).size2();
