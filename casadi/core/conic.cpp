...
namespace casadi {
     ...
     {{"discrete",
      ...
      {"condense",
       {OT_BOOL,
        ...
        "Requires stage-local H, explicit dynamics with a -I next-state block, "
        "and equality bounds on dynamics rows. Supported by DAQP and QRQP."}},
      {"structure_detection",
       {OT_STRING,
      ...
      {"nx",
       {OT_INTVECTOR,
        "Positive number of states per stage, length N+1 (manual structure_detection)"}},
      {"nu",
       {OT_INTVECTOR,
        "Nonnegative number of controls per stage, length N (manual structure_detection)"}},
      {"ng",
       {OT_INTVECTOR,
      ...
      {"condensed_block_count",
       {OT_INT,
        ...
        "estimate a suitable partition. k > 0 fixes n_hat=k. "
      ...
      {"condense_partition_strategy",
       {OT_STRING,
        ...
        "'uniform' estimates a block count and splits the horizon equally; "
        "'optimal' minimizes sum (nx + sum nu)^3 in O(N^2) time, "
        "or O(condensed_block_count*N^2) for a fixed block count."}},
      {"debug",
       {OT_BOOL,
  ...
  void Conic::init(const Dict& opts) {
    ...
    if (condense_) {
      casadi_assert(condensing_support(), "Plugin '" + std::string(plugin_name())
        + "' does not support condensing.");
      casadi_assert(np_ == 0, "Condensing does not support semidefinite constraints.");
      casadi_assert(std::find(discrete_.begin(), discrete_.end(), true) == discrete_.end(),
        "Condensing does not support discrete variables.");
      discrete_.clear();
  ...
  void Conic::set_work(void* mem, const double**& arg, double**& res,
    ...
    if (condense_) {
      // Set up the condensed problem
      casadi_condensing_data<double>& d_cond = m->d_cond;
      d_cond.prob = &p_cond_;
      int flag = casadi_condensing_check(&p_cond_, arg[CONIC_A], arg[CONIC_LBA], arg[CONIC_UBA]);
      casadi_assert(flag != 1, "Condensing requires finite equality bounds on dynamics rows.");
      casadi_assert(flag != 2, "Condensing requires a -I next-state block in dynamics rows.");
      casadi_condensing_set_work(&d_cond, &arg, &res, &iw, &w);

      // Project H and A into stage blocks
      m->fstats.at("block_transform").tic();
      casadi_project(arg[CONIC_H], H_, d_cond.RSQ_val, RSQsp_, w);
      d_cond.g_orig = arg[CONIC_G];
      d_cond.x0_orig = arg[CONIC_X0];
      d_cond.lam_x0_orig = arg[CONIC_LAM_X0];
      d_cond.lam_a0_orig = arg[CONIC_LAM_A0];
      casadi_project(arg[CONIC_A], A_, d_cond.AB_val, ABsp_, w);
      if (CDsp_.nnz() > 0) {
        casadi_project(arg[CONIC_A], A_, d_cond.CD_val, CDsp_, w);
      }
      d_cond.lba_orig = arg[CONIC_LBA];
      d_cond.uba_orig = arg[CONIC_UBA];
      d_cond.lbx_orig = arg[CONIC_LBX];
      d_cond.ubx_orig = arg[CONIC_UBX];
      ...
      d_qp.uba = d_cond.uba;
      d_qp.x0 = d_cond.x;
      d_qp.lam_x0 = d_cond.lam_x;
      d_qp.lam_a0 = d_cond.lam_a;
      d_qp.x = d_cond.x;
  ...
  eval(const double** arg, double** res, casadi_int* iw, double* w, void* mem) const {
    ...
    if (condense_ && !m->d_qp.success) {
      if (res[CONIC_X]) casadi_fill(res[CONIC_X], nx_, nan);
      if (res[CONIC_LAM_X]) casadi_fill(res[CONIC_LAM_X], nx_, nan);
      if (res[CONIC_LAM_A]) casadi_fill(res[CONIC_LAM_A], na_, nan);
      if (res[CONIC_COST]) *res[CONIC_COST] = nan;
    }
  ...
  Dict Conic::get_stats(void* mem) const {
    ...
    if (condense_) stats["condense_partition"] = M_;
    return stats;
  }
  ...
  void Conic::serialize_body(SerializingStream &s) const {
    ...
    // Serialize inputs; rebuild derived condensing data on deserialization.
  ...
  Conic::Conic(DeserializingStream & s) : FunctionInternal(s) {
    ...
    // Rebuild from the stored stage dimensions and partition options.
    if (condense_) {
      build_condense_blocks();
    }
  ...
  void Conic::set_qp_prob() {
    if (condense_) {
      // Pass the condensed sparsities to the solver.
  ...
  // Build sparsity from packed (offset_r, offset_c, rows, cols) blocks.
  ...
  void Conic::detect_condense_structure() {
    if (condense_structure_detection_ == COND_STRUCT_AUTO) {
      ...
      for (casadi_int i = 0; i < AT.size2(); ++i) {
        ...
        casadi_assert(pivot > AT.colind()[i],
          "Automatic OCP detection does not support empty constraint rows; use manual detection.");
      ...
      nxs_.push_back(pivot - start_pivot + 1);
      casadi_assert(!nus_.empty() && !ngs_.empty(),
        "Could not detect OCP dynamics; use manual structure detection.");
      // Correction for k==0
      nxs_[0] = A_skyline[0];
      nus_[0] = 0;
      ...
      casadi_assert(N_ >= 1 && N_ <= na_, "OCP horizon must satisfy 1 <= N <= na.");
  ...
  void Conic::build_condense_blocks() {
    casadi_assert(N_ >= 1 && N_ <= na_, "OCP horizon must satisfy 1 <= N <= na.");
    casadi_assert(nxs_.size() == N_+1 && nus_.size() == N_+1 && ngs_.size() == N_+1,
      "OCP stage dimensions do not match the horizon.");
    casadi_int nz = 0, nc = 0;
    for (casadi_int k = 0; k <= N_; ++k) {
      casadi_assert(nxs_[k] > 0 && nxs_[k] <= nx_
        && nus_[k] >= 0 && nus_[k] <= nx_ && ngs_[k] >= 0 && ngs_[k] <= na_,
        "OCP dimensions require positive nx and nonnegative nu and ng.");
      casadi_assert(nxs_[k] <= nx_-nz, "OCP variable dimensions exceed the QP size.");
      nz += nxs_[k];
      casadi_assert(nus_[k] <= nx_-nz, "OCP variable dimensions exceed the QP size.");
      nz += nus_[k];
      casadi_int gap = k < N_ ? nxs_[k+1] : 0;
      casadi_assert(gap >= 0 && gap <= na_-nc, "OCP constraint dimensions exceed the QP size.");
      nc += gap;
      casadi_assert(ngs_[k] <= na_-nc, "OCP constraint dimensions exceed the QP size.");
      nc += ngs_[k];
    }
    casadi_assert(nz == nx_ && nc == na_ && nus_[N_] == 0,
      "OCP stage dimensions must match the QP dimensions, with nu[N] = 0.");
    casadi_assert(condensed_block_count_ >= 0 && condensed_block_count_ <= N_,
      "condensed_block_count must be between 0 and N.");
    // Derive and validate the partition
    if (M_.empty()) {
      derive_condense_partition();
    }
    casadi_assert(M_.size() >= 2, "Partition M must contain at least two endpoints.");
    ...
    casadi_assert(H_.is_subset(RSQsp_), "Condensing requires a stage-block-diagonal Hessian.");
    DM gap_sp(na_, nx_);
    gap_nz_.clear();
    for (casadi_int k = 0; k < N_; ++k) {
      casadi_int row = AB_blocks_packed_[4*k];
    ...
    casadi_assert(A_.is_subset(ABsp_.unite(CDsp_).unite(gap_sp.sparsity())),
      "Constraint sparsity does not match the specified OCP stage structure.");
  ...
  void Conic::derive_condense_partition() {
    // Check state-dimension uniformity
  ...
  // Choose a block count and split the horizon uniformly.
  void Conic::frison_uniform_partition() {
    casadi_int n_hat = condensed_block_count_;
    if (n_hat == 0) {
      // Estimate the block count from average stage dimensions.
  ...
  // Minimize sum_K (nx[M[K]] + sum_j nu[j])^3 by dynamic programming.
  void Conic::dp_optimal_partition() {
    ...
    // With a free block count, the shortest path through stage boundaries costs O(N^2).
    if (condensed_block_count_ == 0) {
      std::vector<double> cost(N_+1, inf);
      std::vector<casadi_int> prev(N_+1, -1);
      cost[0] = 0;
      for (casadi_int b = 1; b <= N_; ++b) {
        for (casadi_int a = 0; a < b; ++a) {
          double c = cost[a] + blk_cost(a, b);
          if (c < cost[b]) {
            cost[b] = c;
            prev[b] = a;
          }
      ...
      M_.clear();
      for (casadi_int b = N_; b >= 0; b = prev[b]) M_.push_back(b);
      std::reverse(M_.begin(), M_.end());
      return;
    }
    casadi_int K_max = condensed_block_count_;
    ...
    casadi_int n_hat = condensed_block_count_;
    casadi_assert(f[n_hat][N_] != INF, "Could not construct the requested condensing partition.");
  ...
  // Unpack block descriptors and initialize the runtime problem.
  void Conic::finalize_condense_prob() {
    ...
    p_cond_.gap_nz = get_ptr(gap_nz_);
  ...
  void Conic::qp_codegen_body(CodeGenerator& g) const {
    ...
    if (condense_) {
      ...
      g << "d_qp.success = 0;\n";
      for (auto&& output : std::vector<std::pair<casadi_int, casadi_int>>{
          {CONIC_X, nx_}, {CONIC_LAM_X, nx_}, {CONIC_LAM_A, na_}, {CONIC_COST, 1}}) {
        std::string dst = "res[" + str(output.first) + "]";
        g << "if (" << dst << ") " << g.fill(dst, output.second, g.constant(nan)) << "\n";
      }
      g.add_auxiliary(CodeGenerator::AUX_PROJECT);
      g.add_auxiliary(CodeGenerator::AUX_COPY);

      // Store dimensions and length-prefixed block descriptors in the constant pool.
      ...
      // Unpack the original block descriptors.
      ...
      g << "p_cond.gap_nz = " << g.constant(gap_nz_) << ";\n";
      g << "casadi_condensing_setup(&p_cond);\n";

      g << "if (casadi_condensing_check(&p_cond, arg[" << CONIC_A
        << "], arg[" << CONIC_LBA << "], arg[" << CONIC_UBA << "])) return 1;\n";
      ...
      // Project H and A and assign the remaining inputs.
      g << g.project("arg[" + str(CONIC_H) + "]", H_,
                     "d_cond.RSQ_val", RSQsp_, "w") << "\n";
      g << "d_cond.g_orig = arg[" << CONIC_G << "];\n";
      g << "d_cond.x0_orig = arg[" << CONIC_X0 << "];\n";
      g << "d_cond.lam_x0_orig = arg[" << CONIC_LAM_X0 << "];\n";
      g << "d_cond.lam_a0_orig = arg[" << CONIC_LAM_A0 << "];\n";
      ...
      g << "d_qp.x0 = d_cond.x;\n";
      g << "d_qp.lam_x0 = d_cond.lam_x;\n";
      g << "d_qp.lam_a0 = d_cond.lam_a;\n";

      g << "d_qp.f = &d_cond.cost;\n";
      g << "d_qp.x = d_cond.x;\n";
  ...
  void Conic::qp_codegen_post(CodeGenerator& g) const {
    if (!condense_) return;
    // Lift and copy the primal and dual solution.
    g.add_auxiliary(CodeGenerator::AUX_COPY);
    g << "casadi_condensing_lift(&d_cond);\n";
    g << "if (res[" << CONIC_COST << "]) *res[" << CONIC_COST << "] = d_cond.cost;\n";
