    ...
    switch (op) {
      ...
      case OP_MUL:
        if (is_equal(y, x, depth))
          ...
          return -(x * y.dep());
        else if (unique_x && x.is_op(OP_ADD) && x.dep(1).is_op(OP_DIV) && is_equal(x.dep(1).dep(1), y, depth))
          return x.dep(0)*y + x.dep(1).dep(0); // (a + b/c)*c -> a*c + b
        else if (unique_x && x.is_op(OP_ADD) && x.dep(0).is_op(OP_DIV) && is_equal(x.dep(0).dep(1), y, depth))
          return x.dep(0).dep(0)+x.dep(1)*y; // (a/c + b)*c -> a*c + b
        else if (unique_y && y.is_op(OP_ADD) && y.dep(1).is_op(OP_DIV) && is_equal(y.dep(1).dep(1), x, depth))
          return x*y.dep(0) + y.dep(1).dep(0); // c*(a + b/c) -> c*a + b
        else if (unique_y && y.is_op(OP_ADD) && y.dep(0).is_op(OP_DIV) && is_equal(y.dep(0).dep(1), x, depth))
