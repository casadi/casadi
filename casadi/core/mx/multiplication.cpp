...
namespace casadi {
  ...
  void Multiplication::evaluateMX(const MXPtrV& input, MXPtrV& output,
                                  ...
                                  bool output_given) {
    if (!output_given)
      *output[0] = *input[0] + mul(*input[1], *input[2], input[0]->sparsity());
    ...
    for (int d=0; d<nadj; ++d) {
      adjSens[d][1]->addToSum(mul(*adjSeed[d][0], input[2]->T(), input[1]->sparsity()));
      adjSens[d][2]->addToSum(mul(input[1]->T(), *adjSeed[d][0], input[2]->sparsity()));
