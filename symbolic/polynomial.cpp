...
namespace CasADi{
  ...
  Polynomial::Polynomial(const std::vector<double>& coeff) : p_(coeff){
    casadi_assert(!coeff.empty());
  }
  ...
  double Polynomial::toScalar() const{
  ...
  Polynomial& Polynomial::operator-=(const Polynomial& b){
    p_.resize(max(p_.size(),b.p_.size()),0);
    transform(p_.begin(),p_.begin()+b.p_.size(),b.p_.begin(),p_.begin(),std::minus<double>());
    trim();
    return *this;
  }
