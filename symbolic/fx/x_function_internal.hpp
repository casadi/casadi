...
void XFunctionInternal<PublicType,DerivedType,MatType,NodeType>::generateCode(const std::string& src_name){
  ...
  // Codegen the rest of the sparsity patterns REMOVE
  generateSparsityPatterns(gen.sparsities_,gen.added_sparsities_);
  ...
  CodeGenerator::generateCopySparse(cfile);

  // Codegen the dependent functions
  std::map<const void*,int> dependent_index;
  generateDependents(cfile,gen.added_sparsities_,dependent_index);
  ...
  cfile << "  *n_out = " << n_o << ";" << std::endl;
  cfile << "  return 0;" << std::endl;
  cfile << "}" << std::endl << std::endl;

  // Get the sparsity
  cfile << "int getSparsity(int i, int *nrow, int *ncol, int **rowind, int **col){" << std::endl;

  // Get the sparsity index using a switch
  cfile << "  int* sp;" << std::endl;
  ...
  for(int i=0; i<num_io_patterns; ++i){
    ...
    for(it_type it=r.first; it!=r.second; ++it){
      cfile << "    case " << it->second << ":" << std::endl;
    }
    
    // Map to sparsity
    cfile << "      sp = s" << i << "; break;" << std::endl;
  }

  // Finalize the switch
  cfile << "    default:" << std::endl;
  cfile << "      return 1;" << std::endl;
  cfile << "  }" << std::endl << std::endl;
  
  // Decompress the sparsity pattern
  cfile << "  *nrow = sp[0];" << std::endl;
  cfile << "  *ncol = sp[1];" << std::endl;
  cfile << "  *rowind = sp + 2;" << std::endl;
  cfile << "  *col = sp + 2 + (*nrow + 1);" << std::endl;
  cfile << "  return 0;" << std::endl;
  cfile << "}" << std::endl << std::endl;
  
  // Generate the actual function
  generateFunction(cfile, "evaluate", "const d*","d*","d",gen.added_sparsities_,dependent_index);
