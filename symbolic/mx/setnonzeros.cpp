...
namespace CasADi{
  ...
  SetNonzeros::SetNonzeros(const MX& y, const MX& x, const std::vector<int>& nz) : NonzerosBase(nz){
  ...
  void SetNonzeros::evaluateMX(const MXPtrV& input, MXPtrV& output, const MXPtrVV& fwdSeed, MXPtrVV& fwdSens, const MXPtrVV& adjSeed, MXPtrVV& adjSens, bool output_given){
    evaluateMXBase(input,output,fwdSeed,fwdSens,adjSeed,adjSens,output_given,false);
  }
  ...
  void SetNonzeros::generateOperation(std::ostream &stream, const std::vector<std::string>& arg, const std::vector<std::string>& res, CodeGenerator& gen) const{
    ...
    if(Slice::isSlice(nz_)){
      ...
      Slice s(nz_);
      stream << "  for(rr=" << res.front() << "+" << s.start_ << ", ss=" << arg.at(1) << "; rr!=" << res.front() << "+" << s.stop_ << "; rr+=" << s.step_ << ")";
      stream << " *rr = *ss++;" << endl;
