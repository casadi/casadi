...
%fragment("to"{IVector}, "header", fragment="fwd,make_vector") {
  int to_IVector(GUESTOBJECT *p, void *mv, int offs) {
    ...
    return make_vector(p, m, to_IVector);
#endif // SWIGPYTHON
    return false;
  }
}
...
%fragment("to"{DVector}, "header", fragment="fwd,make_vector") {
  int to_DVector(GUESTOBJECT *p, void *mv, int offs) {
    ...
    return make_vector(p, m, to_DVector);
#endif // SWIGPYTHON
    return false;
  }
 }
...
%fragment("to"{GenericType}, "header", fragment="fwd,make_vector", fragment="to"{string}, fragment="to"{IVector}, fragment="to"{DVector}) {
...
%fragment("to"{DMatrix}, "header", fragment="fwd,make_vector") {
  int to_DMatrix(GUESTOBJECT *p, void *mv, int offs) {
    ...
    cleanup: // yes that's right; goto.
      ...
      std::vector <double> t;
      int res = make_vector(p, &t, to_DMatrix);
      if (t.size()>0) {
...
%fragment("to"{IMatrix}, "header", fragment="fwd,make_vector") {
  int to_IMatrix(GUESTOBJECT *p, void *mv, int offs) {
    ...
    if (to_int(p, 0)) {
      ...
      std::vector <int> t;
      int res = make_vector(p, &t, to_IMatrix);
