...
\texttt{MX.fun} works the same way, but for the \texttt{MX} expression graphs:
...
The last line is essentially equivalent to \lstinline[language=Python]{SX.fun('nlp',[x,p],[f,g])}, but is preferred for a number of reasons:
\begin{itemize}
...
\section{Converting \texttt{MX.fun} to \texttt{SX.fun}}
An \texttt{MX.fun} which only contains built-in operations (e.g. elementwise operations such as addition, square root, matrix multiplications and calls to \texttt{SX.fun} instances, can be converted into an \texttt{SX.fun} using the syntax:

\begin{lstlisting}[language=Python]
sx_function = SX.fun(mx_function)
\end{lstlisting}
