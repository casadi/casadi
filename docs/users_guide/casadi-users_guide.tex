...
\begin{itemize}
  ...
  \item (\emph{Python only}) \lstinline[language=Python]{SX(list)}: Create a column vector ($n$-by-1 matrix) with the elements in the list, e.g. \lstinline[language=Python]{SX([1,2,3,4])} (note the difference between Python lists and MATLAB horizontal concatenation, which both uses square bracket syntax)
...
Lookup-tables are supported in \CasADi for rectangular grids, through the \lstinline[language=Python]{interpolant} function:\\
\verb|interpolant(name,plugin,grid,values,options)|,
with \verb|name| an arbitrary label, \verb|plugin| the name of the desired plugin,
\verb|grid| the collection of 1D grids whose outer product defines the full N-D rectangular grid, and \verb|values| the data corresponding to the grid, in a flattened way. Both the \verb|grid| and the \verb|values| must be numerical in nature.

The result of an \lstinline[language=Python]{interpolant} call is a \CasADi Function object that is differentiable, and can be embedded into \CasADi computational graphs by calling with MX arguments. Furthermore, code-generation is fully supported for such graphs.

Currently two plugins exist for \lstinline[language=Python]{interpolant}: \verb|linear| and \verb|bspline|.
The \verb|bspline| variant mimics that of MATLAB's \verb|spline|: not-a-knot end conditions.

In the case of \verb|bspline|, coefficients will be sought at construction time that fit the provided data.
Alternatively, you may also use the more low-level \verb|Function.bspline| to supply the coefficients yourself.
The default degree of the bspline is 3 in each dimension. You may deviate from this default by passing a \verb|degree| option.
...
For reference, we give the Python/Matlab syntax for a spline fit next to the CasADi syntax.
...
interp = scipy.interpolate.InterpolatedUnivariateSpline(xgrid, V)
print(interp(2.5))
...
# Using Scipy
interp = scipy.interpolate.RectBivariateSpline(xgrid, ygrid, data)
...
\begin{itemize}
  \item[] \lstinline[language=Python]{x = opti.variable()}: scalar
  \item[] \lstinline[language=Python]{x = opti.variable(5)}: column vector
  \item[] \lstinline[language=Python]{x = opti.variable(5,3)}: matrix
  \item[] \lstinline[language=Python]{x = opti.variable(5,5,'symmetric')}: symmetric matrix
\end{itemize}
...
\begin{itemize}
  \item[] \lstinline[language=Python]{opti.subject_to( sqrt(x+y) >= 1)}: inequality
  \item[] \lstinline[language=Python]{opti.subject_to( sqrt(x+y) > 1)}: same as above
  \item[] \lstinline[language=Python]{opti.subject_to( 1<= sqrt(x+y) )}: same as above
  \item[] \lstinline[language=Python]{opti.subject_to( 5*x+y==1 )}: equality
\end{itemize}
...
Each \lstinline[language=Python]{subject_to} command adds to the set of constraints in the problem specification.
Use \lstinline[language=Python]{subject_to()} to empty this set and start over.
...
After setting up the problem, you may call the solve method, which constructs a CasADi \lstinline[language=Python]{nlpsol} and calls it.
\begin{lstlisting}[language=Python]
sol = opti.solve()
\end{lstlisting}

You may call \lstinline[language=Python]{solve} any number of times. You will always get an immutable copy of the problem specification and its solution.
Consecutively calling \lstinline[language=Python]{solve} will not help the convergence of the problem.
...
\begin{itemize}
  \item[] \lstinline[language=Python]{sol.value(x)}: value of a decision variable
  \item[] \lstinline[language=Python]{sol.value(p)}: value of a parameter
  \item[] \lstinline[language=Python]{sol.value(sin(x+p))}: value of an expression
  \item[] \lstinline[language=Python]{sol.value(jacobian(opti.g,opti.x))}: value of constraint jacobian
\end{itemize}

Note that the return type of \lstinline[language=Python]{value} is sparse when applicable.

\paragraph{Numerical value at other points}

You may pass a list of overruling assignment expressions to \lstinline[language=Python]{value}.
