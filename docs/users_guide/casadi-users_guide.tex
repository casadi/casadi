...
More usage examples for \texttt{SX} can be found in the tutorials at \htmladdnormallink{http://forum.casadi.org}{http://forum.casadi.org/}. For documentation of particular functions of this class (and others), find the ``C++ API docs'' on the website and search for information about \lstinline[language=C++]{casadi::Matrix<T>}.
...
Function objects are created with the syntax:
\begin{lstlisting}[language=Python]
f = ClassName(name, arguments, ..., [options])
\end{lstlisting}
...
One important member of the \texttt{Function} derived classes is \texttt{SXFunction}, which is constructed by passing a list of input expressions and a list of output expressions:
...
The last line is essentially equivalent to \lstinline[language=Python]{SXFunction('nlp',[x,p],[f,g])}, but is preferred for a number of reasons:
\begin{itemize}
...
\begin{lstlisting}[language=Python]
nlp_solver = NlpSolver(display_name,plugin_name,nlp,options)
\end{lstlisting}
...
\emph{NOTE: The SymbolicOCP class is not yet stable and undergoing refactoring. Fore more information, see \htmladdnormallink{CasADi issue 1358}{https://github.com/casadi/casadi/issues/1358}.}
...
\begin{center}
  ...
  \begin{tabular}{| p{3.5cm} | p{3.5cm} | p{3.5cm} | p{3.5cm} | }
    ...
    *Logical and & \verb|a && b| & \verb|logic_and(a, b)| & \verb|a & b| \\ \hline
    *Logical or & \verb=a || b= & \verb|logic_or(a, b)| & \verb=a | b= \\ \hline
    *Logical not & \verb|!a| & \verb|logic_not(a)| & \verb|~a| \\ \hline
    Round to integer
