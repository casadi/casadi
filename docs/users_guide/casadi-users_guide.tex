...
Here is a list of available options for the \texttt{CodeGenerator} class:
\begin{center}
  \scriptsize
  \begin{tabular}{| p{2.0cm} | p{2.0cm} | p{10.0cm} | }
    \hline
    Option & Default value & Description \\
    \hline
    \texttt{verbose} & true & Include comments in generated code \\
    \texttt{mex} & false & Generate an MATLAB/Octave MEX entry point \\
    \texttt{cpp} & false & Generated code is C++ instead of C \\
    \texttt{main} & false & Generate a command line interface \\
    \texttt{casadi\_real} & \texttt{double} & Floating point type \\
    \texttt{casadi\_int} & \texttt{long long int} & Integer type \\
    \texttt{with\_header} & false & Generate a header file \\
    \texttt{with\_mem} & false & Generate a simplified C API \\
    \texttt{indent} & 2 & Number of spaces per indentation level \\
    \hline
  \end{tabular}
\end{center}
...
take no inputs and return the number of input or outputs (\verb|casadi_int| is an alias for \verb|long long int|).
...
of constant integers (\verb|const casadi_int*|). This is a compact representation
...
of type \verb|const double*|, \verb|double*|, \verb|casadi_int| and \verb|double|,
respectively.
