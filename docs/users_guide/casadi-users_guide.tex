...
\section{Syntax for generating code}
...
typically self-contained\footnote{An exception to this is if the code generation is performed
for a function that contains call to external functions.}, the compilation
-- on Linux/OSX -- can be as easy as issuing:
''\lstinline[language=sh]{gcc -shared gen.c -o gen.so}'' from the command line.
Assuming \verb|gen.c| was created as described in the previous section,
we can then create a \texttt{Function} \texttt{f} as follows:

\begin{minipage}[t]{0.5\textwidth}
\begin{lstlisting}[language=Python]
# Python
c='gcc -shared ff.c -o ff.so'
from os import system;system(c)
f = external('f', './ff.so')
print f(3.14)
\end{lstlisting}
\end{minipage}
...
system('gcc -shared ff.c -o ff.so')
f = external('f', './ff.so')
print f(3.14)
\end{pytexoutput}

We can also rely on \CasADi performing the compilation \emph{just-in-time}
using \CasADi's \texttt{Compiler} class. This is a plugin class, which at the
...
MATLAB MEX function and call from MATLAB. The generated code will take the function
name as the first argument, followed by the function inputs:
