...
\texttt{MX} expressions may contain calls to \texttt{Function}-derived functions. Calling a function object is both done for the numerical evaluation and, by passing symbolic arguments, for embedding a \emph{call} to the function object into an expression graph (cf. also Section~\ref{sec:integrators}).
...
\section{Initial-value problems and sensitivity analysis} \label{sec:integrators}
...
Integrators in CasADi are function objects that are created using \CasADi's \texttt{integrator} function. Different integrators schemes and interfaces are implemented as \emph{plugins}, essentially shared libraries that are loaded at runtime. Integrators are created using the syntax:

\begin{lstlisting}[language=Python]
# Python
x = SX.sym('x')
z = SX.sym('z')
dae = {'x':x, 'z':z, 'ode':-z*x, 'alg':z**2-2}
opts = {'abstol', 1e-8}
...
x = SX.sym('x');
z = SX.sym('z');
dae = struct('x',x,'z',z,'ode',-z*x,'alg',z^2-2);
solver = integrator('solver', 'idas', dae, opts);
\end{lstlisting}
