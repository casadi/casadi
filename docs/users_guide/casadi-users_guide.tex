...
\begin{description}
  ...
  \item[\emph{A}.is_constant()] Are the matrix entries all constant?
  \item[\emph{A}.is_integer()] Are the matrix entries all integer-valued?
\end{description}

The last queries are examples of queries for which \emph{false negative} returns are allowed. A matrix for which \emph{A}.is_constant() is \emph{true} is guaranteed to be constant, but is \emph{not} guaranteed to be non-constant if \emph{A}.is_constant() is \emph{false}. We recommend you to check the API documentation for a particular function before using it for the first time.
