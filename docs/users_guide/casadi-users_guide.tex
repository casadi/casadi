...
An NLP solver in \CasADi is a function that takes the parameter value (\texttt{p}), the bounds (\texttt{lbx}, \texttt{ubx}, \texttt{lbg}, \texttt{ubg}) and a guess for the primal-dual solution (\texttt{x0}, \texttt{lam_x0}, \texttt{lam_g0}) and returns the optimal solution. Unlike integrator objects, NLP solver functions are currently not differentiable functions in \CasADi.
