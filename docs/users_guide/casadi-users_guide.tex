...
%%   \item[\texttt{Sparsity(\emph{nrow,ncol})}]: Create a sparse \emph{nrow}-by-\emph{ncol} pattern
%%   \item[\texttt{Sparsity.diag(\emph{n})}]: Create a square \emph{n}-by-\emph{n} diagonal pattern
%%   \item[\texttt{Sparsity.upper(\emph{nrow,ncol})}]: Create a \emph{nrow}-by-\emph{ncol} upper triangular pattern
%% \end{description}
...
%% \begin{pytex}
%% print DMatrix.ones(Sparsity.lower(3))
%% print DMatrix(Sparsity.lower(3),range(6))
%% \end{pytex}
...
%% \begin{pytex}
%% (Sparsity.lower(4)*Sparsity.rowcol([2,3],range(4),4,4)).spy()
%% \end{pytex}
...
f = x*x
g = log(x)-p
nlp = SXFunction("nlp",nlpIn(x=x,p=p),nlpOut(f=f,g=g))
\end{pytex}
...
\begin{center}
  \begin{tabular}{| p{3.5cm} | p{3.5cm} | p{3.5cm} | p{3.5cm} | }
    ...
    *Elementwise multiplication, division
    & \verb|x*y, x/y| & \verb|x*y, x/y| & \verb|x.*y, x./y| \\ \hline
    Negation
    & \verb|-x| & \verb|-x| & \verb|-x| \\ \hline
    Natural exponential function
    & \verb|exp(x)| & \verb|exp(x)| & \verb|exp(x)| \\ \hline
    Natural logarithm
    & \verb|log(x)| & \verb|log(x)| & \verb|log(x)| \\ \hline
    *Exponentiation & \verb|x**y| & \verb|pow(x,y)| & \verb|x^y| or \verb|x.^y| \\ \hline
    Square root & \verb|sqrt(x)| & \verb|sqrt(x)| & \verb|sqrt(x)| \\ \hline
    ...
    *Horizontal concatenation
    & \verb|horzcat([x, y])| & \verb|horzcat(x, y)| (max 4 arguments)) or \verb|horzcat(v)| (vector of matrices) & \verb|[x, y]| \\ \hline
    *Vertical concatenation
    & \verb|vertcat([x, y])| & \verb|\verb|vertcat(v)| (vector of matrices) & \verb|[x; y]| \\ \hline
