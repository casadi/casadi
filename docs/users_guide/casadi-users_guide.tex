...
\begin{itemize}
  ...
  \item \python{SX(matrix_type)}: Create a matrix given a numerical matrix given as a \emph{numpy} or \emph{scipy} matrix (in Python) or as a dense or sparse matrix (in MATLAB/Octave). In MATLAB/Octave e.g.
...
Lookup-tables are supported in \CasADi for rectangular grids, through the \python{interpolant} function:\\
...
The result of an \python{interpolant} call is a \CasADi Function object that is differentiable, and can be embedded into \CasADi computational graphs by calling with MX arguments. Furthermore, code-generation is fully supported for such graphs.

Currently two plugins exist for \python{interpolant}: \verb|linear| and \verb|bspline|.
