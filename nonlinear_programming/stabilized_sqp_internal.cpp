...
namespace CasADi{
  ...
  StabilizedSQPInternal::StabilizedSQPInternal(const FX& nlp) : NLPSolverInternal(nlp){
...
//std::max(std::max(norm_inf(gf_),std::max(norm_inf(x_),std::max(norm_inf(s_),1.))));
    ...
    while(true){
      ...
      if (!callback_.isNull()) {
        ...
        int ret = callback_(shared_from_this<FX>(),user_data_);
...
// Calculate candidate Merit function
  ...
  void StabilizedSQPInternal::eval_jac_g(const std::vector<double>& x, std::vector<double>& g, Matrix<double>& J){
    try{
  ...
  void StabilizedSQPInternal::eval_grad_f(const std::vector<double>& x, double& f, std::vector<double>& grad_f){
    try {
      // Get function
      FX& gradF = this->gradF();
