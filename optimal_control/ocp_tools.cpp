...
class EquationSorter{
  public:
    ...
    std::vector<int> eqindex_;
    std::vector<int> varindex_;
    
    // Index to some node reachable from the node
    std::vector<int> eqlowlink_;
    std::vector<int> varlowlink_;
    ...
    // Color edge 
    void decreaseVar(int j);
    void decreaseEq(int i);
  ...
  protected:
    ...
    std::vector<int> eqcount_;
    std::vector<int> varcount_;
    
    // Equations with no uncolored edge
    std::queue<int> singularEq_;
    
    // Equations with exactly one uncolored edge
    std::queue<int> causalEq_;
    
    // Variables with no uncolored edge
    std::queue<int> singularVar_;

    // Variables with exactly one uncolored edge
    std::queue<int> causalVar_;
    
    // The order of the equation (-1 if not assigned)
    std::vector<int> orderEq_;
    
    // The order of the variable (-1 if not assigned)
    std::vector<int> orderVar_;
...
EquationSorter::EquationSorter(const std::vector<SX>& x, const std::vector<SX>& xdot, const std::vector<SX>& z, const std::vector<SX>& dae) : x_(x), xdot_(xdot), z_(z), dae_(dae){
  ...
  IT_ = I_.transpose(mapping_);

  // Initialize indices to -1 (unassigned)
  eqindex_.resize(I_.size1(),-1);
  varindex_.resize(I_.size2(),-1);
  
  // Index to some node reachable from the node
  eqlowlink_.resize(I_.size1(),-1);
  varlowlink_.resize(I_.size2(),-1);
  
  // Depth-first index
  index_ = 0;

  // Count the number of occurences of each equation and variable
  eqcount_.resize(I_.size1(),0);
  varcount_.resize(I_.size2(),0);
  
  // Loop over rows
  for(int i=0; i<I_.size1(); ++i){
    // Loop over non-zero elements
    for(int el=I_.rowind(i); el<I_.rowind(i+1); ++el){
      // column
      int j = I_.col(el);
  ...
  for(int i=0; i<eqcount_.size(); ++i){
    if(eqcount_[i]==0)
      singularEq_.push(i);
    else if(eqcount_[i]==1)
      causalEq_.push(i);
  }
    
  // Add variables to queue
  for(int j=0; j<varcount_.size(); ++j){
    if(varcount_[j]==0)
      singularVar_.push(j);
    else if(varcount_[j]==1){
      causalVar_.push(j);
    }
  }
  
  // Equation assignment vector
  assign_.resize(I_.size2(),-1);
  num_unassigned_ = assign_.size();

  // Allocate edges
  edges_.resize(I_.size(),0);
  ...
  orderEq_.resize(neq,-1);
  orderVar_.resize(nvar,-1);
  min_eq_ = -1;
  max_eq_ = neq;
...
void EquationSorter::decreaseEq(int i){
  eqcount_[i]--;
  if(orderEq_[i]==-1){
    if(eqcount_[i]==0)
      singularEq_.push(i);
...
void EquationSorter::decreaseVar(int j){
  varcount_[j]--;
  if(orderVar_[j]==-1){
    if(varcount_[j]==0)
      singularVar_.push(j);
...
void EquationSorter::colorRed(int i, int j, int eq_no){
  ...
  for(int el=IT_.rowind(j); el<IT_.rowind(j+1); ++el){
...
void EquationSorter::strongconnect(int i){
  
}
  
void EquationSorter::tarjanCellier(){
  cout << "A" << eqcount_ <<  ", "<< varcount_ << endl;
  ...
  cout << orderEq_ << endl;
  cout << orderVar_ << endl;
  cout << "B" << eqcount_ <<  ", "<< varcount_ << endl;
...
/*  vector<int> mapping;
  CRSSparsity IT = I_.transpose(mapping);*/
