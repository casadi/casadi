...
function test_slice_strided_open(M) {
  // Python-style open/strided slice strings on the index proxy:
  // x["0::3"], x[":4"], x["5:"], x["::2"] mirror x[0::3] etc. in Python.
  const v = M.DM([0, 1, 2, 3, 4, 5, 6, 7, 8]);
  assertEqual(String(v["0::3"]), "[0, 3, 6]", "0::3");
  assertEqual(String(v["1::3"]), "[1, 4, 7]", "1::3");
  assertEqual(String(v[":4"]), "[0, 1, 2, 3]", ":4");
  assertEqual(String(v["5:"]), "[5, 6, 7, 8]", "5:");
  assertEqual(String(v["::2"]), "[0, 2, 4, 6, 8]", "::2");
  const m = M.DM([[1, 2, 3], [4, 5, 6]]);
  assertEqual(String(m["0,:"]), "[[1, 2, 3]]", "row slice");
  assertEqual(String(m[":,1"]), "[2, 5]", "column slice");
  // slice assignment
  const w = M.DM([0, 0, 0, 0]);
  w["1:3"] = M.DM([9, 9]);
  assertEqual(String(w), "[0, 9, 9, 0]", "slice assign");
}
...
const tests = [
  ...
  ["test_slice_strided_open", test_slice_strided_open],
];
