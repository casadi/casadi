...
function test_sx_jacobian_scalar(M) {
  ...
  const f = M.Function("df", [x], [df], null);
  const outs = f.call([M.DM(1)], false, false);
  assertArrayAlmostEqual(outs[0].nonzeros(), [Math.cos(1)], 8, "d(sin)/dx at 1");
}
...
function test_mx_jacobian_scalar(M) {
  ...
  const outs = f.call([M.DM(3)], false, false);
  assertArrayAlmostEqual(outs[0].nonzeros(), [6], 8, "d(x^2)/dx at 3");
}
...
function test_sx_gradient_vector(M) {
  ...
  const f = M.Function("g", [x, y], [gx, gy], null);
...
function test_mx_jacobian_vector(M) {
  ...
  const f_sym = M.vertcat([M.times(x0, x1), M.plus(x0, x1)]);
  const J = M.jacobian(f_sym, x);
  assertEqual(J.size1(), 2n, "J size1");
  assertEqual(J.size2(), 2n, "J size2");

  const f = M.Function("J", [x], [J], null);
  // At x = [3, 5]: J = [[5, 3], [1, 1]].
  // Column-major: [5, 1, 3, 1].
  const xv = M.DM([3, 5]);
  const outs = f.call([xv], false, false);
  assertArrayAlmostEqual(outs[0].nonzeros(), [5, 1, 3, 1], 8, "J at (3,5)");
}
...
function test_sx_jacobian_of_gradient(M) {
  ...
  const f = M.Function("H", [x], [H], null);
  const outs = f.call([M.DM(2)], false, false);
  assertArrayAlmostEqual(outs[0].nonzeros(), [48], 8, "d2(x^4)/dx2 at 2");
}
