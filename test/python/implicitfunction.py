...
try:
  ...
  solvers.append(("nlp",{"linear_solver": "csparse", "nlpsol": "ipopt"}))
except:
  pass
try:
  LinearSolver.loadPlugin("csparse")
...
class ImplicitFunctiontests(casadiTestCase):
  ...
  def test_large(self):
    for Solver, options in solvers:
      ...
      solver=f.nlsol("options2", Solver, options2)
  ...
  def testKINSol1c(self):
    ...
    x=SX.sym("x")
    f=Function("f", [x],[sin(x)])
    solver=f.nlsol("solver", "kinsol", {"constraints":[-1]})
    solver.setInput(-6)
    solver.evaluate()
  ...
  def test_implicitbug(self):
    ...
    ifcn = vfcn_sx.nlsol("ifcn", "newton", {"linear_solver":"csparse"})
