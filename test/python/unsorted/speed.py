...
for sol in solvers:
  ...
  for i in range(rep):
    ...
    if sol=="casadi":
      M = csr_matrix((M.nonzeros_double(),M.col(),M.rowind()),(M.size1(),M.size2()),dtype=float)
...
M2_C_S = csr_matrix((M2_C.nonzeros_double(),M2_C.col(),M2_C.rowind()),(M2_C.size1(),M2_C.size2()),dtype=float)
M2_DIFF = M2_C_S-M2_S
