...
class OCPtests(casadiTestCase):
  ...
  def test_singleshooting2(self):
    ...
    q0   = vertcat([var[0],par])
    parl  = var[1]
    [qend,_] =integrator.call([q0,parl,MX(2,1)])
  ...
  def testMSclass_prim(self):
    ...
    xp0 = ssym("x0",nx)
    xf = x0 + p[0]
    daeres = SXFunction({'NUM': DAE_NUM_IN, DAE_T: t,DAE_X: x0,DAE_P: p, DAE_XDOT: xp0},{'NUM': DAE_NUM_OUT, DAE_RES: xf})
  ...
  def testMSclassSimple(self):
    ...
    yd=ssym("yd",3,1)
    p=SX("p")
    f=SXFunction({'NUM':DAE_NUM_IN, DAE_T: t, DAE_X: y, DAE_P: p,DAE_XDOT: yd},[[y[1,0],-y[0,0],p*y[0,0]]])
    f.init()
