...
class casadiTestCase(unittest.TestCase):
  ...
  @contextmanager
  def assertOutputs(self,included_stdout,included_stderr):
    with capture_stdout() as result:
      yield
    if not(isinstance(included_stdout,list)):
      included_stdout = [included_stdout]
    if not(isinstance(included_stderr,list)):
      included_stderr = [included_stderr]
    for e in included_stdout:
      self.assertTrue(e in result[0],msg=result[0] + "<->" + e)
    for e in included_stderr:
      self.assertTrue(e in result[1],msg=result[1] + "<->" + e)
  ...
  def do_codegen(self,F,inputs=None, opts=None,std="c89",extralibs="",check_serialize=False,extra_options=None,main=False,with_external=True,main_return_code=0,valgrind=False,helgrind=False,definitions=None,with_jac_sparsity=False,external_opts=None,vectorize_check=1,compiler=None,static=False,digits=15,includes=None,with_reverse=False,with_forward=False,extra_include=[],digits=15,debug_mode=False,main_output_check=True):
    ...
    if args.run_slow:
      ...
      Fout = F.call(inputs)
      if not static or with_external:
