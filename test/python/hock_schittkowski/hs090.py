...
def hs090():
    ...
    f_opt = 1.36265681
    x_opt = cs.DM([0.717044, 0, 0.8, -0.456611])
    x = cs.MX.sym('x', 4)
    x0 = cs.DM.zeros(4, 1)
    ...
    rhoaux_4[0:30] = 1*emx_1[0:30]*emx_2[0:30]*emx_3[0:30]*emx_4[0:30] - 2*emx_2[0:30]*emx_3[0:30]*emx_4[0:30] + 2*emx_3[0:30]*emx_4[0:30] - 2*emx_4[0:30] + 1
    rho[0:30] = (-1)*rhoaux_4[0:30]/mu[0:30]**2
    aux1[0:30] = A[0:30]**2*rhoaux_4[0:30]**2
    aux3[0:30] = A[0:30]*rho[0:30]
    hsum[0:30] = aux1[0:30]*aux2[0:30] + aux3[0:30]*aux4[0:30]

    ubg[0] = 0.0001
    g[0] = 2/15 + cs.sum1(hsum)
    f = x[0]**2 + x[1]**2 + x[2]**2 + x[3]**2 - x[1] + (x[2] - 8/10)**2
