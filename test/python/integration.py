...
class Integrationtests(casadiTestCase):
  ...
  def test_tools_trivial(self):
    ...
    for integrator in [
      ...
      solution = SX.fun("solution", [x,p,tf],[x*exp(tf)], {"input_scheme":["x0","p","h"], "output_scheme":["xf"]})
  ...
  def test_tools(self):
    ...
    t0=SX.sym("t0")
    q0=SX.sym("q0")
    f=SX.fun("f", [vertcat((q,t)),p],[vertcat((q/p*t**2,1))])
    for integrator in [
            simpleRK(f,500),
            simpleIRK(f,50),
            simpleIntegrator(f)
    ...
    for tt in checks():
      for p_features, din, dout, rdin, rdout,  solutionin, solution, point, (tstart_, tend_) in variations(*tt):
        for Integrator, features, options in integrators:
          ...
          x = SX.sym("x")
          dummyIntegrator = Function.integrator("dummyIntegrator", Integrator, SX.fun("dae", daeIn(x=x), daeOut(ode=x)))
  ...
  def setUp(self):
    ...
    x=SX.sym("x")
    p=SX.sym("p")
    f=SX.fun("f", daeIn(t=t, x=x, p=p),daeOut(ode=x/p*t**2))
    opts = {}
    opts["reltol"] = 1e-15
    opts["abstol"] = 1e-15
    opts["fsens_err_con"] = True
  ...
  def test_eval2(self):
    self.message('CVodes integration: evaluation with MX.fun indirection')
    num=self.num
    qend=self.qend
    
    par=self.par
    q0=self.q0
    qe=MX.fun("qe", [q0,par],[qend[0]])
    
    f = MX.fun("f", [q0],qe([q0,MX(num['p'])]))
    f.setInput([num['q0']],0)
    f.evaluate()
  ...
  def test_issue92c(self):
    ...
    y=SX.sym("y")
    z=x*exp(t)
    f=SX.fun("f", daeIn(t=t, x=vertcat([x,y])),[vertcat([z,z])])
    # Pass inputs
    f.setInput(1.0,"t")
  ...
  def test_issue92b(self):
    ...
    x=SX.sym("x")
    y=SX.sym("y")
    f=SX.fun("f", daeIn(t=t, x=vertcat([x,y])),daeOut(ode=vertcat([x,(1+1e-9)*x])))
    opts = {}
    opts["fsens_err_con"] = True
    opts["t0"] = 0
    opts["tf"] = 1
  ...
  def test_issue92(self):
    ...
    dq=vertcat([x,x])
    f=SX.fun("f", daeIn(t=t,x=q),daeOut(ode=dq))
    opts = {}
    opts["fsens_err_con"] = True
    opts["reltol"] = 1e-12
    opts["t0"] = 0
  ...
  def test_bug_repeat(self):
    ...
    dh = p+q[0]**2
    f=SX.fun("f", daeIn(x=q,p=p,t=t),daeOut(ode=vertcat([dh ,q[0],dh])))
    ...
    f=SX.fun("f", daeIn(x=q,p=p,t=t),daeOut(ode=vertcat([dh ,q[0],(1+1e-9)*dh])))
  ...
  def test_hess5(self):
    ...
    JT = MX.fun("JT", [q0,p],[MX.jac(qe, 1,0)[0].T])
  ...
  def test_linear_system(self):
    ...
    f=SX.fun("f", daeIn(t=t,x=q,p=p),daeOut(ode=mul(c.reshape(p,3,3),q)))
    opts = {}
    opts["fsens_err_con"] = True
    opts["reltol"] = 1e-15
    opts["abstol"] = 1e-15
  ...
  def test_mathieu_system(self):
    ...
    f=SX.fun("f", daeIn(x=q,p=p,t=t),daeOut(ode=vertcat([q[1],(p[0]-2*p[1]*cos(2*p[2]))*q[0]])))
    opts = {}
    opts["fsens_err_con"] = True
    opts["reltol"] = 1e-15
    opts["abstol"] = 1e-15
  ...
  def test_nl_system(self):
    ...
    qeJf=MX.fun("qeJf", [q0,par],[vec(qeJ([q0,par])[0])])
  ...
  def test_hessian2D(self):
    ...
    ode = SX.fun("ode", daeIn(x=x, p=vec(A)),daeOut(ode=mul(A,x)))
    I = Function.integrator("I", "cvodes", ode, {"fsens_err_con": True, 'reltol' : 1e-12})
    I.setInput(x0_,"x0")
    I.setInput(vec(A_),"p")
    I.evaluate()
    ...
    JT = MX.fun("JT", [q0,p],[MX.jac(qe, 1,0).T])
  ...
  def test_issue535(self):
    ...
    rz=SX.sym("rz")
    rp=SX.sym("rp")
    f = SX.fun("f", daeIn(**{'x': x, 'z': z}),daeOut(**{'alg': x-z, 'ode': z}))
    g = SX.fun("g", rdaeIn(**{'x': x, 'z': z, 'rx': rx, 'rz': rz}),rdaeOut(**{'alg': x-rz, 'ode': rz}))

    integrator = Function.integrator("integrator", "idas", (f,g), {'calc_ic': True, 'tf': 2.3, 'reltol': 1e-10, 'augmented_options': {'reltol': 1e-09, 'abstol': 1e-09 }, 'calc_icB': True, 'abstol': 1e-10, 't0': 0.2})
