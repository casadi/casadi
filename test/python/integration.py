...
class Integrationtests(casadiTestCase):
      ...
      for tt in checks():
        ...
        for p_features, din, dout, rdin, rdout, solutionin, solution, point, (tstart_, tend_) in variations(*tt):
          if p_features[0] in features:
            ...
            opts["tf"] = tend_
            if Integrator=='idas':
              opts["init_xdot"] = DM(point["x0"]).nonzeros_double()
              opts["calc_icB"] = True
  ...
  def test_hess3(self):
    ...
    q0=num['q0']
    p=num['p']
    self.assertAlmostEqual(H_out["ddxf_dp_dp"][0],(q0*tend**6*exp(tend**3/(3*p)))/(9*p**4)+(2*q0*tend**3*exp(tend**3/(3*p)))/(3*p**3),9,"Evaluation output mismatch")
