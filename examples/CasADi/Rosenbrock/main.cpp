 ...
 *
 * The actual problem is generated by CasADi in `codegen-rosenbrock.py`, this 
 * example shows how to load the problem and solve it using PANOC-ALM.
 */
...
int main(int argc, char *argv[]) {
    ...
    auto soname = "examples/CasADi/Rosenbrock/librosenbrock_functions.so";
    if (argc > 1)
        soname = argv[1];
    ...
    // Load the functions
    p.f           = load_CasADi_objective(soname);
    p.grad_f      = load_CasADi_gradient_objective(soname);
    p.g           = load_CasADi_constraints(soname);
    p.grad_g_prod = load_CasADi_gradient_constraints_prod(soname);
