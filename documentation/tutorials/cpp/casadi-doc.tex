...
\newcommand{\codediv}[2]{\HTMLcode[ID="#1"]{DIV}{#2}}
...
\tableofcontents
\begin{rawhtml}
<p style="background-color: rgb(120,120,120);">valami</p>
\end{rawhtml}
\end{latexonly}
...
what the dependencies of each are.
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Package/interface} & \textbf{Dependency}\\
\hline
CasADi core & g++ compiler\\
\hline
\end{tabular}
\end{center}
...
%\subsection{The SXFunction class}
%\subsection{The MXFunction class}
\begin{align}
\dot{x} = f(x(t), u(t))
\end{align}
\section{Forward and backward derivatives}
In this section we will learn how to calculate first-order derivatives of general functions. Within CasADi derivatives are 
calculated with automatic differentiation, which has two modes forward and backward.
...
\par
TODO: WRITE ABOUT jac(), jacobian(), hessian()
...
interface to state-of-the-art ODE solvers \cite{Hindmarsh2005, Serban2005} like CVODES and IDAS from the Sundials suite.
...
In the following subsection we would like to introduce the most important integrator options. Most of the options are directly available and usable
 known from the CVODES documentation, altough this is very detailed and quite often one cannot find what he is looking for.
 \begin{center}
 \small
