...
\title{User Documentation for \CasADi v1.9.0}
\begin{document}
%\htmlinfo*
%\sffamily
\titlepage
...
Though it is possible and somtimes benificial, we shall not work with the \texttt{SX} type directly in this tutorial. Instead we shall work with a \emph{sparse matrix type} called \texttt{SX}, whose elements are \texttt{SX} instances. \texttt{SX} uses an everything-is-a-matrix type syntax that should be familiar to Matlab users, which means that scalars can be represented as 1-by-1 matrices and vectors as $n$-by-1 matrices. To store its element, it uses a general sparse matrix format comparible to that used for sparse matrices in Matlab\footnote{To be more precise, the storage format used is \emph{compressed row storage}}.
...
Note that the string passed is only the display name, not the identifier. Multiple variables can have the same name, but still be different. You can also create vectors or matrices of variables using by supplying additional arguments to \texttt{SX.sym}:
\begin{pytex}
y = SX.sym("y",5)   # A 5-by-1 matrix, i.e. a vector, with symbolic variables
Z = SX.sym("Z",4,2) # A 4-by-2 matrix with symbolic variables
\end{pytex}

Note that \texttt{SX.sym} is a function which returns an \texttt{SX} instance. When variables have been declared, expressions can now be formed in an intuitive way:
\begin{pytex}
...
You can also create \texttt{SX} instances \emph{without} any symbolic variables\footnote{In C++, class}:
\begin{pytex} 
...
\texttt{DMatrix} is very similar to \texttt{SX} (in fact, they are just two template instantiations of the same C++ class \texttt{CasADi::Matrix<\,T\,>}), but with the difference that the nonzero elements are numerical values and not \texttt{SX} expressions. The syntax is also the same, except for functions such as \texttt{SX.sym} or \texttt{jacobian} which has no equivalent.
...
\begin{pytex}
print daeOut()
print daeOut(ode=SX.sym("fx"))
print daeOut(alg=SX.sym("fz"),ode=SX.sym("fx"))
\end{pytex}
...
where \texttt{MX.sym} has replaced \texttt{SX.sym}.
...
\texttt{MX} supports getting and setting elements, using the same syntax as \texttt{SX}, but the way it is implemented is very different. Test, for example, to print the first column of a 2-by-2 symbolic variable:
\begin{pytex}
x = MX.sym("x",2,2)
print x[:,0] 
\end{pytex}
