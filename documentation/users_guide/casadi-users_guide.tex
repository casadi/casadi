...
The output should be understood as a (linear) mapping from nonzeros 0 and 2 of the expression \texttt{x} to a new expression of dimension 1-by-2.
...
One important member of the \texttt{FX} derived classes is \texttt{SXFunction}, which is constructed with the syntax \verb|SXFunction(list_of_inputs,list_of_outputs)|:
\begin{pytex}
x = SX.sym("x",2,2)
y = SX.sym("y")
...
We can get a list of other supported function objects deriving from \texttt{FX}:
\begin{pytex}
import casadi
import casadi.tools
casadi.tools.print_subclasses(casadi.FX)
...
All derived classes of \texttt{FX} come with a range of options that can be set. The API documentation shows the list of available options, along with their default values and descriptions. For example, here is an excerpt from the documentation for \texttt{IpoptSolver}:
\begin{pytexoutput}
...
\texttt{MX} expressions may contain calls to \texttt{FX}-derived functions. To embed a call to function (e.g. \texttt{SXFunction} or \texttt{CVodesIntegrator}, see Section \ref{chapter:integrators}), you use the syntax:
\begin{verbatim}
...
\begin{pytex}
print DMatrix(Sparsity.tril(3),1)
