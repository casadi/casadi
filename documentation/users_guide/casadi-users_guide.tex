...
CasADi offers a variety of different ways to get or set specific elements or blocks of elements in matrix types. All slicing techniques fall into one of the following two categories:
...
Use this to target individual elements. The \emph{nonzero access} flavour uses one number, the non-zero index, in rectangular brackets. The \emph{full access} flavour uses two.
...
\subsection{Slice access}

Slices are the most common way to access a range of elements at once.
...
\begin{verbatim}
 ...
 print M
 print M[:]    # [3,7,8,9,4,5,6,1]
 print M[1:]   # [  7,8,9,4,5,6,1]
 print M[:-1]  # [3,7,8,9,4,5,6  ]
 print M[1:4]  # [  7,8,9        ]
 print M[::3]  # [3,    9,    6  ]
 print M[1,:]  # [[4,  5,  6,  1 ]]
 print M[:,0]  # [[3],[4]]
 print M[0,1]  # 7
 print M[:,1:] # [[7,8,9],[5,6,1]]
\end{verbatim}

\subsection{List access}

Being similar to slices in effect, lists are a more flexible way to access a group of elements at once.
...
\begin{verbatim}
 ...
 print M[[6,1,3]]      # [6,7,9]
 print M[0,[0,3]]      # [[3,  9]]
 print M[[1,0],[0,3]]  # [[4,  1],[3,  9]]
\end{verbatim}

\subsection{Simple IMatrix access}

This indexing type does repeated scalar access with a custom result shape. The user specifies \texttt{IMatrix}'es as item(s) in the rectangular brackets.  The result takes over the shape/sparsity of the supplied \texttt{IMatrix}.
...
\begin{verbatim}
 ...
 print DMatrix(sp_tril(3),range(21))  # [[1,  00,  00 ], [2,  3,  00 ], [4,  5,  6 ]]
\end{verbatim}
...
To see how to use the Modelica import, look at \htmladdnormallink{thermodynamics\_example.py}{http://sourceforge.net/apps/trac/casadi/browser/trunk/examples/python/modelica/fritzson_application_examples/thermodynamics_example.py} and \htmladdnormallink{cstr.cpp}{http://sourceforge.net/apps/trac/casadi/browser/trunk/examples/cplusplus/cstr.cpp} in CasADi's example collection. 
...
\section{The FlatOCP class} \label{sec:modelica_import}
The symbolic optimal control problem representation (or just model description) contained in \texttt{modelDescription.xml} can be imported into CasADi in the form of the FlatOCP class:
\begin{verbatim}
ocp = FlatOCP('modelDescription.xml')
\end{verbatim}

The FlatOCP class contains symbolic representation of the optimal control problem designed to be general and allow manipulation. The basic usage of this class is first to set options that influences the parsing of the XML file, initialize it by calling \texttt{ocp.init()} and then calling the different class methods to access or manipulate the optimal control problem. For a description of this class and its method, we refer to the API documentation.
