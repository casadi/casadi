...
\CasADi allows the user to create function objects, also knowns as \emph{functors}, that all derive the \texttt{FX} base class. This includes functions that are defined by a symbolic expression, ODE/DAE integrators, QP solvers, NLP solvers etc.
...
All derived classes of \texttt{FX} come with a range of options that can be set. The API documentation shows the list of available options for each class, along with their default values and descriptions.
