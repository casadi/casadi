...
void WorhpInternal::init(){
  ...
  if (GF_.output().size()==0) {
    ...
    MX fake(GF_.output().size1(),GF_.output().size2());
    fake(0,0) = 0;
    
    GF_ = MXFunction(GF_.symbolicInput(),fake);
    GF_.init();
...
void WorhpInternal::checkinit() {
  ...
  // Check if temprary freeG equals previous freeG and set dirty if so
  ...
  // Create modified G that does not contain the non-free X, nor the free G
  if (!G_.isNull()) {
    std::vector< MX > Gmod_in = G_.symbolicInput();
  ...
  if (!H_.isNull()) {
    ...
    Hmod_ = MXFunction(Hmod_in,H_.call(H_in).at(0));
    Hmod_.init(); 
  }
  
  // Create modified J that does not contain the non-free X, nor the free G
  if (!J_.isNull()) {
    std::vector< MX > Jmod_in = J_.symbolicInput();
  ...
  if (!GF_.isNull()) {
    std::vector< MX > GFmod_in = GF_.symbolicInput();
...
bool WorhpInternal::eval_h(const double* x, double obj_factor, const double* lambda, double* values){
  try{
    ...
    if(monitored("eval_h")){
      ...
      Hmod_.setInput(x);
      if(n_hess_in>1){
        Hmod_.setInput(lambda,  1);
        Hmod_.setInput(obj_factor,  2);
      }
...
bool WorhpInternal::eval_g(const double* x, double* g)
{
  try {
    ...
    if(worhp_o.m>0){
      ...
      // Printing
      if(monitored("eval_g")){
        cout << "x = " << Gmod_.input() << endl;
        cout << "g = " << Gmod_.output() << endl;
      }
    }
...
bool WorhpInternal::eval_grad_f(const double* x,double scale , double* grad_f )
{
  try {
    ...
    if(GFmod_.isNull()){
      ...
      Fmod_.setAdjSeed(scale);

      // Evaluate, adjoint mode
      Fmod_.evaluate(0,1);

      // Get the result
      Fmod_.getAdjSens(grad_f);
