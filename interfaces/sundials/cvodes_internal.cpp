...
void CVodesInternal::init(){
  ...
  if(nq_>0){
    ...
    yQ0_ = N_VMake_Serial(nq_,&argument(INTEGRATOR_X0)[ny_]);
...
// Sensitivities
  if(ad_order_>0){
    ...
    if(finite_difference_fsens_){
      ...
      flag = CVodeSetSensParams(mem_,&argument(INTEGRATOR_P)[0],0,0);
...
void CVodesInternal::rhs(double t, const double* y, double* ydot){
  ...
  f_.setInput(t,ODE_T);
  f_.setInput(y,ODE_Y);
  f_.setInput(argument(INTEGRATOR_P),ODE_P);
...
void CVodesInternal::integrate(double t_out){
  ...
  if(fabs(t_-t_out)<ttol){
    copy(argument(INTEGRATOR_X0).begin(),argument(INTEGRATOR_X0).end(),result(INTEGRATOR_XF).begin());
    if(fsens_order_>0){
      for(int i=0; i<nfdir_; ++i){
...
// Pass input
  ...
  q_.setInput(argument(INTEGRATOR_P),ODE_P);
...
void CVodesInternal::psetup(double t, N_Vector y, N_Vector fy, booleantype jok, booleantype *jcurPtr, double gamma, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3){
  ...
  M_.setInput(argument(INTEGRATOR_P),M_P);
