...
import sys
import warnings
from ..casadi_generator import generate_casadi_problem, generate_casadi_control_problem, generate_casadi_quadratic_control_problem, generate_experimental_casadi_control_problem
...
def _load_experimental_casadi_control_problem(sofile, N):
    print("-- Loading:", sofile)
    prob = pa.load_experimental_casadi_control_problem(sofile, N=N)
    return prob

def generate_and_compile_experimental_casadi_control_problem(
    N: int,
    f: cs.Function,
    l: cs.Function,
    l_N: cs.Function,
    ...
    with shelve.open(cachefile) as cache:
        if key in cache:
            ...
            sofile = join(probdir, soname)
            try:
                return _load_experimental_casadi_control_problem(sofile, N)
            except:
                del cache[key]
                # if os.path.exists(probdir) and os.path.isdir(probdir):
                #     shutil.rmtree(probdir)
        ...
        os.makedirs(builddir, exist_ok=True)
        probdir = join(cachedir, str(uid))
        cgen = generate_experimental_casadi_control_problem(f, l, l_N, h, h_N, c, c_N, name)
        cfile = cgen.generate(join(projdir, ""))
        with open(join(projdir, 'CMakeLists.txt'), 'w') as f:
            f.write(f"""
                cmake_minimum_required(VERSION 3.17)
