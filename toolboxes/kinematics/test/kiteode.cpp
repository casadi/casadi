...
SXMatrix ode(const SX &t_,const SXMatrix &q_,const SXMatrix &p_,const SXMatrix &d_,const SXMatrix &u_,SXMatrix &h_) {
  	...
  	// <<DEFINE STATES
	SXMatrix q(7,1);for (int i=0;i<7;i++) q[i]=q_[i];
	SXMatrix dq(7,1);for (int i=0;i<7;i++) dq[i]=q_[7+i];
  ...
  SXMatrix	ddelta	=dq[0];
  SXMatrix	dr	=dq[1];
  SXMatrix	dphi	=dq[2];
  SXMatrix	dtheta	=dq[3];
  SXMatrix	dR	=dq[4];
  SXMatrix	dP	=dq[5];
  SXMatrix	dY	=dq[6];
  ...
  SXMatrix ddq; ddq << dddelta << ddr <<ddphi <<ddtheta << ddR << ddP <<ddY;
  ...
  KinVec T(CR*qaer*A_w*b_w,CP*qaer*A_w*c_w,CY*qaer*A_w*b_w,0,f4);	// aero torque
